...
class TextEncoder(nn.Module):
    def __init__(self, input_dim, embed_dim, output_dim, seq_len, data_path, freeze_pretrained=True):
...
class MotionTextClassifier(nn.Module):
    def __init__(self, motion_input_dim, text_input_dim, embed_dim, text_output_dim, hidden_size, num_classes, seq_len):
        super(MotionTextClassifier, self).__init__()
        self.motion_encoder = MotionEncoder(motion_input_dim, 256, 4)
        self.text_encoder = TextEncoder(text_input_dim, embed_dim, text_output_dim, seq_len, data_path="D:\\MingYuan\\Dataset\\beat_v2.0.0\\beat_english_v2.0.0", freeze_pretrained=True)
    ...
    def forward(self, motion, text):
        ...
        print("cat_view_shape", combined_features.shape)
        output = self.fc(combined_features)
        return output, motion_features.size(1)
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        with self.lmdb_env.begin(write=False) as txn:
            ...
            #     result = chardet.detect(sample.read(10000))
            #     encoding = result['encoding']
            # print(f"Detect encoding:{encoding}")

            # with open(sample, 'r', encoding=encoding) as sample:
            #     sample = sample.read()
                # pickle.loads(sample.read())
            # sample = pickle.load(sample)
            ...
            # pose, _, _, word, _, sem = sample
            # # pose = torch.from_numpy(pose.copy()).float()
            # # pose = torch.from_numpy(pose).float()
            # # word = torch.from_numpy(word.copy()).int()
            # pose = torch.from_numpy(pose.copy()).float()  # 确保数组可写
            # word = torch.from_numpy(word).int()
            # # sem = torch.from_numpy(sem.copy()).float()
            # sem = torch.from_numpy(sem).float()
            # label = 2 if sem.mean().item() > 0.1 else (1 if sem.mean().item() > 0 else 0)
            # return pose, word, label
...
#     poses, _, texts, _, _, labels = zip(*batch)
#     # 对poses进行填充

#     # 使用 .clone().detach() 而不是 torch.tensor 来复制张量
#     # poses = [torch.tensor(pose, dtype=torch.float32) for pose in poses]
#     poses = [pose.clone().detach().float() for pose in poses]
...
#         text_tensor[i, :len(text)] = text

#     return poses_padded, text_tensor, torch.tensor(labels, dtype=torch.long)
def collate_fn(batch):
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, output_path, log_path):
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, _, texts, _, _, labels in train_loader:
            try:
                inputs, texts, labels = inputs.cuda(), texts.cuda(), labels.cuda()
                optimizer.zero_grad()
                outputs, seq_len = model(inputs, texts)
                outputs = outputs.view(-1, outputs.shape[-1])
                labels = labels.unsqueeze(1).repeat(1, seq_len).view(-1)  # 调整标签形状以匹配输入
...
def evaluate_model(model, dataloader, criterion):
    ...
    with torch.no_grad():
        for inputs, _, texts, _, _, labels in dataloader:
            inputs, texts, labels = inputs.cuda(), texts.cuda(), labels.cuda()
...
# def main(npz_input_path, textgrid_input_path, txt_input_path, output_path, log_path, max_length):
def main(output_path, log_path):
    ...
    motion_input_dim = 165  # motion dim
    ...
    model = MotionTextClassifier(motion_input_dim, text_input_dim, embed_dim, text_output_dim, hidden_size, num_classes, seq_len).cuda()
