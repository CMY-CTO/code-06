...
class MotionTextClassifier(nn.Module):
    def __init__(self, motion_input_dim, text_input_dim, embed_dim, text_output_dim, hidden_size, num_classes, depth=4, dropout_rate=0.5):
        super(MotionTextClassifier, self).__init__()
        self.motion_encoder = nn.Sequential(
            MotionEncoder(motion_input_dim, 256, 4),
            nn.Dropout(dropout_rate) # Dropout
        )
        self.text_encoder = nn.Sequential(
            TextEncoder(text_input_dim, embed_dim, text_output_dim, data_path="D:\\MingYuan\\Dataset\\beat_v2.0.0\\beat_english_v2.0.0", freeze_pretrained=True),
            nn.Dropout(dropout_rate)  # Dropout
        )
        self.decoder = UNetDecoder(512, depth, dropout_rate)  # 512 为通道数

        # 添加全连接层
        self.fc = nn.Sequential(
            nn.Linear(512, hidden_size),  # 512 是 decoder 输出的通道数
            nn.ReLU(),
    ...
    def forward(self, motion, text):
        ...
        output = output.reshape(-1, 512)  # 512 是 decoder 输出的通道数
...
def main(output_path, log_path):
    ...
    class Args:
        ...
        loader_type = "train"
        preloaded_dir = "D:\\MingYuan\\A2M\\code\\classifier\\datasets_cache"
        training_speakers = [2]
    ...
    val_dataset = CustomDataset(args, "val", build_cache=True)
    test_dataset = CustomDataset(args, "test", build_cache=True)
