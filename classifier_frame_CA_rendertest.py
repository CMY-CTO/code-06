...
class CrossAttention(nn.Module):
    ...
    def forward(self, text_features, motion_features):
        ...
        K = self.key_proj(motion_features)
        V = self.value_proj(motion_features)
...
class MotionTextClassifier(nn.Module):
    def __init__(self, motion_input_dim, motion_output_dim, text_input_dim, embed_dim, text_output_dim, hidden_size, num_classes, data_path, UNetDecoder_depth=4, dropout_rate=0.5, period=32, max_seq_len=600):
        ...
        # 动态设置 d_model
        self.d_model = max(motion_output_dim, text_output_dim)

        self.motion_encoder = nn.Sequential(
            MotionEncoder(motion_input_dim, self.d_model, 4),
            nn.Dropout(dropout_rate)
        )
        self.text_encoder = nn.Sequential(
            TextEncoder(text_input_dim, embed_dim, text_output_dim, data_path=data_path, freeze_pretrained=True),
            nn.Linear(text_output_dim, self.d_model),
            nn.Dropout(dropout_rate)
        )
        self.positional_encoding = PeriodicPositionalEncoding(d_model=self.d_model, period=period, max_seq_len=max_seq_len)
        self.cross_attention = CrossAttention(d_model=self.d_model)
    ...
    def forward(self, motion, text):
        ...
        # 添加位置编码
        motion_features = self.positional_encoding(motion_features)
        text_features = self.positional_encoding(text_features)

        # cross_attention
        attended_features = self.cross_attention(text_features, motion_features)

        combined_features = attended_features.permute(0, 2, 1)
...
def test_model(model, dataloader, criterion, num_classes,args):
    ...
    # args         # Arguments containing paths and settings
    # )
...
if __name__ == "__main__":
    root = "D:\\MingYuan\\A2M"
    output_path = os.path.join(root, "MTClassifier-frame_CA_M2T_motion128text4_period32—rendertest")
