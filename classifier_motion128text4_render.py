...
from render.rendering import visualize_motion_samples
...
class TextEncoder(nn.Module):
    def __init__(self, input_dim, embed_dim, output_dim, data_path, freeze_pretrained=True):
        ...
        self.conv = nn.Sequential(
            nn.Conv1d(embed_dim, 4, kernel_size=3, stride=1, padding=1),  # Change 256 to 32
            nn.ReLU(),
            nn.Conv1d(4, 4, kernel_size=3, stride=1, padding=1),  # Change 256 to 32
            nn.ReLU(),
            nn.Conv1d(4, 4, kernel_size=3, stride=1, padding=1),  # Change 256 to 32
            nn.ReLU(),
            nn.Conv1d(4, output_dim, kernel_size=3, stride=1, padding=1),  # Change 256 to 32
            nn.ReLU()
        )
...
# 填充输入序列：在输入序列的末尾添加 0 以使其长度变为最近的 2 的次方
def pad_to_power_of_two(seq, base=2):
    """
    将序列填充到最近的2的次方长度
    """
    length = seq.size(2)
    target_length = int(base ** np.ceil(np.log2(length)))
    pad_size = target_length - length
    if pad_size > 0:
        seq = F.pad(seq, (0, pad_size))
    return seq, pad_size

# 移除填充部分：在模型输出后移除填充部分，以恢复原始的序列长度
def remove_padding(seq, pad_size):
    """
    移除填充的部分
    """
    if pad_size > 0:
...
class UNetDecoder(nn.Module):
    def __init__(self, feature_dim, depth=4, dropout_rate=0.5):
        ...
        for i in range(depth):
            self.down_layers.append(
                nn.Sequential(
                    nn.Conv1d(feature_dim, feature_dim, kernel_size=3, stride=2, padding=1),
                    nn.LeakyReLU(0.2, inplace=True),
            ...
            print(f"Added down layer {i}: feature_dim={feature_dim}")

        # 创建上采样路径
        for i in range(depth):
            in_channels = feature_dim * 2 if i > 0 else feature_dim
            self.up_layers.append(
                nn.Sequential(
                    nn.Conv1d(in_channels, feature_dim, kernel_size=3, stride=1, padding=1),
                    nn.LeakyReLU(0.2, inplace=True),
                    nn.Conv1d(feature_dim, feature_dim, kernel_size=3, stride=1, padding=1),
                    nn.LeakyReLU(0.2, inplace=True),
            ...
            print(f"Added up layer {i}: in_channels={in_channels}, feature_dim={feature_dim}")

    def forward(self, x):
        def align_tensors(x1, x2):
            # x1 是上采样后的张量，x2 是要连接的张量
            if x1.size(2) > x2.size(2):
                x1 = x1[:, :, :x2.size(2)]
            elif x1.size(2) < x2.size(2):
                pad_size = x2.size(2) - x1.size(2)
                x1 = F.pad(x1, (0, pad_size))
            return x1
        ...
        for i, layer in enumerate(self.up_layers):
            if i > 0:
                x = align_tensors(x, down_features[-(i + 1)])
                x = torch.cat([x, down_features[-(i + 1)]], dim=1)
            x = layer(x)
            print(f"After up layer {i}: shape={x.shape}")

        return x

class MotionTextClassifier(nn.Module):
    def __init__(self, motion_input_dim, motion_output_dim, text_input_dim, embed_dim, text_output_dim, hidden_size, num_classes, data_path, UNetDecoder_depth=4, dropout_rate=0.5, period=32, max_seq_len=600):
        super(MotionTextClassifier, self).__init__()

        # 动态设置 d_model
        self.d_model = motion_output_dim + text_output_dim

        self.motion_encoder = nn.Sequential(
            MotionEncoder(motion_input_dim, motion_output_dim, 4),
            nn.Dropout(dropout_rate) # Dropout
        )
        self.text_encoder = nn.Sequential(
            TextEncoder(text_input_dim, embed_dim, text_output_dim, data_path=data_path, freeze_pretrained=True),
            nn.Dropout(dropout_rate)  # Dropout
        )
        self.decoder = UNetDecoder(self.d_model, UNetDecoder_depth, dropout_rate)
    ...
    def forward(self, motion, text):
        ...
        # 进行转置以符合 nn.Conv1d 的输入要求
        combined_features = combined_features.permute(0, 2, 1)
        # 填充输入序列到最近的2的次方长度
        combined_features, pad_size = pad_to_power_of_two(combined_features)
        print("before Unet", combined_features.shape)
        output = self.decoder(combined_features)
        print("after Unet", output.shape)
        # 移除填充部分
        output = remove_padding(output, pad_size)
        print("after remove padding", output.shape)
        # 转置回来以匹配 MotionTextClassifier 的输出
...
class CustomDataset(Dataset):
    def __init__(self, args, loader_type, augmentation=None, kwargs=None, build_cache=True):
        ...
        # print('!!![[[loader_type',loader_type,'!!![[[selected_file', self.selected_file )
    ...
    def calculate_joint_positions(self):
        ...
        for tar in os.listdir(dir_p):
            if tar.endswith(".npz"):
                ...
                for i in range(s):
                    with torch.no_grad():
                        joints = self.smplx(
                            ...
                            body_pose=poses[i*max_length:(i+1)*max_length, 3:213+3],
                            left_hand_pose=poses[i*max_length:(i+1)*max_length, 253:403],
                            right_hand_pose=poses[i*max_length:(i+1)*max_length, 403:553],
                            return_verts=True,
                            return_joints=True,
                ...
                if r != 0:
                    with torch.no_grad():
                        joints = self.smplx(
                            ...
                            body_pose=poses[s*max_length:s*max_length+r, 3:213+3],
                            left_hand_pose=poses[s*max_length:s*max_length+r, 253:403],
                            right_hand_pose=poses[s*max_length:s*max_length+r, 403:553],
                            return_verts=True,
                            return_joints=True,
                ...
                all_positions.append(joints.cpu().numpy())

        return np.concatenate(all_positions, axis=0)

    def calculate_velocity(self, positions, save_path):
        dt = 1/30
        joints = torch.from_numpy(positions).permute(1, 0, 2)  # Shape: (55, n, 3)
        ...
        vel_seq = torch.cat([init_vel, middle_vel, final_vel], dim=1).permute(1, 0, 2)  # Shape: (n, 55, 3)
        
        np.save(save_path, vel_seq.cpu().numpy())
    ...
    def cache_generation(self, out_lmdb_dir, disable_filtering, clean_first_seconds,  clean_final_seconds, is_test=False):
        ...
        for index, file_name in self.selected_file.iterrows():
            ...
            if "smplx" in self.args.pose_rep:
                ...
                exps = pose_data["expressions"]
                
                n, c = poses.shape[0], poses.shape[1]
                betas = np.tile(betas.reshape(1, 300), (n, 1))
                betas = torch.from_numpy(betas).cuda().float()
                poses = torch.from_numpy(poses.reshape(n, c)).cuda().float()
                exps = torch.from_numpy(exps.reshape(n, 100)).cuda().float()
                trans = torch.from_numpy(trans.reshape(n, 3)).cuda().float()
                ...
                positions = torch.cat(positions, axis=0).cpu().numpy()

                # Calculate velocities
                dt = 1/30
                joints = torch.from_numpy(positions).permute(1, 0, 2)
                init_vel = (joints[:, 1:2] - joints[:, :1]) / dt
                middle_vel = (joints[:, 2:] - joints[:, :-2]) / (2 * dt)
                final_vel = (joints[:, -1:] - joints[:, -2:-1]) / dt
                velocities = torch.cat([init_vel, middle_vel, final_vel], dim=1).permute(1, 0, 2).cpu().numpy()

                # Concatenate positions and velocities with pose data
                pose_each_file = np.concatenate([pose_each_file, positions, velocities], axis=1)
    ...
    def __getitem__(self, idx):
        with self.lmdb_env.begin(write=False) as txn:
            ...
            if self.loader_type == "test":
                ...
                labels[sem > 0.1] = 1
                # labels[(sem == 0.5) | (sem == 0.6) | (sem == 0.7)] = 1
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, output_path, log_path, num_classes, grad_clip_val=1.0, weight_decay=1e-4):
    ...
    for epoch in range(num_epochs):
                ...
                for group in optimizer.param_groups:
                    for param in group['params']:
                        if param.grad is not None:
                            param.grad.data.add_(param.data * weight_decay)
        ...
        if val_acc > best_val_acc:
            ...
            # 保存模型到 wandb
            # wandb.save(os.path.join(output_path, 'best_motion_text_classifier.pth'))
...
def plot_confusion_matrix(cm, epoch, output_path):
    ...
    plt.close()
    # 上传到 WandB
    wandb.log({f"Confusion Matrix Epoch {epoch}": wandb.Image(cm_path)})
...
def evaluate_model(model, dataloader, criterion, num_classes, output_path, epoch):
    ...
    plot_confusion_matrix(cm, epoch, output_path)
    ...
    # 记录验证指标到 wandb
    wandb.log({
        "Val Loss": running_loss / len(dataloader),
        "Val Accuracy": acc,
...
def test_model(model, dataloader, criterion, num_classes,args):
    model.eval()
    tracker = other_tools.EpochTracker(["loss", "accuracy", "precision", "recall", "f1"], [False, True, True, True, True])
    tracker.reset()
    running_loss = 0.0
    ...
    with torch.no_grad():
        for batch in dataloader:
            try:
                ...
                inputs = batch['pose'].cuda() # (1, seq_len, 169)
                betas = batch['beta'].cuda()
                words = batch['word'].cuda() # (1, seq_len)
                sems = batch['sem'].cuda()
                trans = batch['trans'].cuda()
                labels = batch['label'].cuda()  # (1, seq_len)

                # print(f"Test batch labels: {labels}")
                # print(f"Max label: {labels.max()}, Min label: {labels.min()}")

                outputs = model(inputs, words) # (1, seq_len, num_classes)
                ...
                preds = torch.argmax(outputs, dim=1)
                # 打印批次信息
                print(f"Batch labels shape: {labels.shape}, preds shape: {preds.shape}")
                all_labels.extend(labels.cpu().numpy())
                all_preds.extend(preds.cpu().numpy())
                print(f"all_labels_length{len(all_labels)},all_preds_length{len(all_preds)}")

                # 将所有批次的数据传递给 `visualize_motion_samples`
                visualize_motion_samples(
                    all_labels,
                    all_preds,
                    [batch],
                    args
    ...
    # 记录测试指标到 wandb
    wandb.log({
        "Test Loss": running_loss / len(dataloader),
        "Test Accuracy": acc,
...
class TemporalSmoothingCrossEntropy(nn.Module):
    def __init__(self, smoothing=0.1, kernel_size=5, sigma=1.0):
        super(TemporalSmoothingCrossEntropy, self).__init__()
    ...
    def forward(self, pred, target):
        ...
        # 重新调整 smooth_target 的形状
        smooth_target = torch.full((n_samples, n_classes), self.smoothing / (n_classes - 1), device=pred.device)
        ...
        # 平滑在样本维度上
        smooth_target = smooth_target.view(-1, 1, n_classes)  # (batch_size*seq_len, 1, num_classes)
        smooth_target = F.conv1d(smooth_target, self.gaussian_kernel, padding=self.padding)
        smooth_target = smooth_target.view(-1, n_classes)  # 转换回 (batch_size*seq_len, num_classes)

        loss = -torch.sum(smooth_target * F.log_softmax(pred, dim=1), dim=1).mean()
        return loss
...
def main(output_path, log_path):
    ...
    wandb.init(project=config.wandb.project, entity=config.wandb.entity, config=config_dict)

    # 数据集准备
    dataset_start_time = time.time()
    train_dataset = CustomDataset(config.dataset, "train", build_cache=False)
    val_dataset = CustomDataset(config.dataset, "val", build_cache=False)
    test_dataset = CustomDataset(config.dataset, "test", build_cache=False)
    ...
    # 模型和优化器
    # data_path, depth=4, dropout_rate=0.5
    model = MotionTextClassifier(
        config.model.motion_dim,
        config.model.motion_output_dim,
        config.model.text_dim,
        config.model.embed_dim,
        config.model.text_output_dim,
        config.model.hidden_size,
        config.model.num_classes,
        config.dataset.data_path,
        config.model.UNetDecoder_depth,
        config.model.dropout_rate,
        config.model.period,
        config.model.max_seq_len
    ).cuda()

    criterion = TemporalSmoothingCrossEntropy(smoothing = config.criterion.label_smoothing,
                                              kernel_size= config.criterion.kernel_size,
                                              sigma= config.criterion.sigma)
    ...
    test_metrics = test_model(model, test_loader, criterion, config.model.num_classes,config.render)
...
if __name__ == "__main__":
    root = "D:\\MingYuan\\A2M"
    output_path = os.path.join(root, "classifier_frame_motion128_text4_render_0816")
